  ...
  return (
    ...
    >
      <Link to="/food/$id" params={{ id: food.id }} className="relative block">
        <img src={food.image} alt={food.name} loading="lazy" width={512} height={512}
      ...
      <div className="p-3 space-y-2 flex-1 flex flex-col">
        <div className="flex items-start gap-2">
          <span className={`mt-1 w-3.5 h-3.5 border-2 grid place-items-center shrink-0 ${food.veg ? "border-success" : "border-destructive"}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${food.veg ? "bg-success" : "bg-destructive"}`} />
          </span>
          <Link to="/food/$id" params={{ id: food.id }} className="font-semibold text-sm leading-tight flex-1 hover:text-primary transition">
            {food.name}
          </Link>
          {food.rating && (
        ...
        <div className="flex items-center justify-between pt-1 mt-auto">
          ...
          <Button size="sm" className="rounded-md h-8 gap-1 px-3 text-xs font-bold" onClick={() => { addToCart(food.id); toast.success(`${food.name} added to cart`); }}>
