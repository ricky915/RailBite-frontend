  ...
  return (
    <section className="rounded-2xl bg-gradient-to-r from-[oklch(0.97_0.04_55)] to-[oklch(0.96_0.06_45)] border p-6 md:p-8 grid md:grid-cols-3 items-center gap-6">
      <div>
        ...
        <div className="text-5xl font-extrabold text-primary leading-none">{offer.percent}% OFF</div>
      ...
      <div>
        <h3 className="text-xl md:text-2xl font-bold leading-tight">
          {offer.sub.split(" ").slice(0, 2).join(" ")}<br />
        ...
        <div className="grid grid-cols-3 gap-3 mt-4 text-xs">
          {[
            ...
            <div key={t} className="flex flex-col items-center gap-1 text-primary font-semibold text-center">
              <I className="w-5 h-5" /><span>{t}</span>
            </div>
          ))}
        </div>
      </div>
