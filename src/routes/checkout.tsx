...
import { z } from "zod";
import { CreditCard, Smartphone, Wallet, Banknote, CheckCircle2, MapPin, Loader2 } from "lucide-react";
...
const schema = z.object({
  ...
  pnr: z.string().trim().regex(/^[0-9]{10}$/, "10-digit PNR"),
...
function CheckoutPage() {
  const { cart, menu, cartTotal, placeOrder, currentUser } = useStore();
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          const place =
            data.locality || data.city || data.principalSubdivision || `${latitude.toFixed(3)}, ${longitude.toFixed(3)}`;
          setValue("station", place, { shouldValidate: true });
          toast.success(`Location detected: ${place}`);
        } catch {
          setValue("station", `${latitude.toFixed(3)}, ${longitude.toFixed(3)}`, { shouldValidate: true });
  ...
  const onSubmit = (data: Form) => {
    const order = placeOrder({
      ...
      items: cart.map((c) => {
        const f = menu.find((m) => m.id === c.id)!;
        return { id: c.id, name: f.name, qty: c.qty, price: f.price };
      }),
      total: grand,
      pnr: data.pnr,
      coach: data.coach,
  ...
  if (placed) {
    return (
      <div className="max-w-xl mx-auto py-16 text-center px-4">
        ...
        <p className="text-muted-foreground mt-2">Your order <b>{placed}</b> is being prepared.</p>
        <div className="flex gap-3 justify-center mt-6">
          <Button asChild variant="outline"><Link to="/orders">My Orders</Link></Button>
          <Button asChild><Link to="/track">Track Order</Link></Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 grid lg:grid-cols-[1fr_360px] gap-6">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <section className="bg-card border rounded-2xl p-5 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <Field label="Email" error={errors.email?.message} full><Input type="email" {...register("email")} /></Field>
          </div>
        </section>
        ...
        <section className="bg-card border rounded-2xl p-5 space-y-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <Button type="button" size="sm" variant="outline" onClick={detectLocation} disabled={locating} className="rounded-full">
              {locating ? <Loader2 className="w-4 h-4 mr-1.5 animate-spin" /> : <MapPin className="w-4 h-4 mr-1.5" />}
              {locating ? "Detecting..." : "Use my location"}
            </Button>
          </div>
          <div className="grid md:grid-cols-2 gap-3">
            <Field label="PNR (10 digits)" error={errors.pnr?.message}><Input maxLength={10} {...register("pnr")} /></Field>
            <Field label="Station / Delivery Location" error={errors.station?.message}><Input placeholder="e.g. Kanpur Central" {...register("station")} /></Field>
            <Field label="Coach" error={errors.coach?.message}><Input placeholder="e.g. B3" {...register("coach")} /></Field>
            <Field label="Seat" error={errors.seat?.message}><Input placeholder="e.g. 42" {...register("seat")} /></Field>
          </div>
        </section>
        ...
        <section className="bg-card border rounded-2xl p-5 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">This is a demo checkout — no real payment will be charged.</p>
        </section>
      </form>
      ...
      <aside className="bg-card border rounded-2xl p-5 h-fit sticky top-[88px] space-y-3">
        ...
        {cart.map((c) => {
          ...
          return (
            <div key={c.id} className="flex justify-between text-sm">
              <span className="truncate">{f.name} × {c.qty}</span>
              <span>₹{f.price * c.qty}</span>
            </div>
          );
        })}
        ...
        <div className="border-t pt-3 flex justify-between font-bold text-lg"><span>Total</span><span>₹{grand}</span></div>
        <Button size="lg" className="w-full rounded-full h-12" onClick={handleSubmit(onSubmit)}>
          Place Order (₹{grand})
        </Button>
      </aside>
    </div>
