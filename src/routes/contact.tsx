...
export const Route = createFileRoute("/contact")({
  head: () => ({ meta: [{ title: "Contact – SRFOOD" }, { name: "description", content: "Get in touch with SRFOOD support." }] }),
    ...
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 grid md:grid-cols-2 gap-6">
      <div>
        ...
        <div className="mt-6 space-y-4">
          <Info I={Mail} label="Email" v={content.contactEmail} />
          <Info I={Phone} label="Phone" v={content.contactPhone} />
          <Info I={MapPin} label="Address" v={content.contactAddress} />
        </div>
      </div>
      <form
        onSubmit={handleSubmit((d) => { toast.success(`Thanks ${d.name}, we'll respond shortly.`); reset(); })}
...
function Info({ I, label, v }: { I: React.ComponentType<{ className?: string }>; label: string; v: string }) {
  return (
    <div className="flex gap-3">
      <div className="w-10 h-10 rounded-lg bg-primary/10 text-primary grid place-items-center"><I className="w-4 h-4" /></div>
      <div><div className="text-xs uppercase tracking-wide text-muted-foreground">{label}</div><div className="font-medium">{v}</div></div>
    </div>
  );
}
