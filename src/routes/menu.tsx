...
export const Route = createFileRoute("/menu")({
  head: () => ({
    meta: [
      { title: "Menu – SRFOOD" },
      { name: "description", content: "Browse our full menu of thalis, biryanis, snacks, beverages and desserts." },
    ],
  }),
  component: MenuPage,
});
    ...
    <div className="px-4 md:px-6 py-5 max-w-[1400px] mx-auto animate-fade-in">
      <div className="flex flex-col gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Fresh, hygienic meals — delivered to your seat.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(["all", "veg", "nonveg"] as Diet[]).map((d) => (
            <Button
              key={d} size="sm" variant={diet === d ? "default" : "outline"}
              onClick={() => setDiet(d)}
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 border-2 border-dashed rounded-2xl">
          ...
          <p className="text-sm text-muted-foreground mt-1">Try clearing the search or switching categories.</p>
        </div>
      ) : (
